...
int main()
{
    ...
    if (0 == rank % 2)
    {
        MPIS_Send_init(&send_buf, 1, MPI_INT, 1, 0, MPI_COMM_WORLD, MPI_INFO_NULL,
                       &my_reqs[0]);
        MPIS_Recv_init(&recv_buf, 1, MPI_INT, 1, 0, MPI_COMM_WORLD, MPI_INFO_NULL,
                       &my_reqs[1]);
    }
    else
    {
        MPIS_Recv_init(&recv_buf, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_INFO_NULL,
                       &my_reqs[0]);
        MPIS_Send_init(&recv_buf, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_INFO_NULL,
                       &my_reqs[1]);
    }
    ...
    // Match
    MPIS_Queue_match(my_queue, &my_reqs[0], MPI_STATUS_IGNORE);
    MPIS_Queue_match(my_queue, &my_reqs[1], MPI_STATUS_IGNORE);
    MPIS_Queue_match(my_queue, &barrier_req, MPI_STATUS_IGNORE);
