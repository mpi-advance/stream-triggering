// Test that creates then frees many objects (basically as if it was only non-blocking and
// not persistant)
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < iterations; ++i)
    {
        ...
        std::cout << rank << " starting iteration: " << i << std::endl;
        // Make requests
        MPIS_Request my_reqs[2];
        if (0 == rank % 2)
        {
            MPIS_Send_init(send_buf, BUFFER_SIZE, MPI_INT, rank+1, 0, MPI_COMM_WORLD, mem_info,
                           &my_reqs[0]);
            MPIS_Recv_init(recv_buf, BUFFER_SIZE, MPI_INT, rank+1, 0, MPI_COMM_WORLD, mem_info,
                           &my_reqs[1]);
        }
        else
        {
    ...
    std::cout << rank << " done with test" << std::endl;
