...
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/ccs/home/dschafer/apps/stream_trigger/lib
...
run_test()(
    ...
    srun --network=job_vni,res_tles=1024 --time=$TIME --nodes=$NODES --ntasks-per-node=$PPN --output=$RUN_FILE --gpus-per-task=1 "../execs/${TEST_NAME}_${SYSTEM}_$1" $NUM_ITERS $BUFF_SIZE
